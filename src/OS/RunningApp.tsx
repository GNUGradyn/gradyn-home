...
const RunningApp = (props: RunningAppProps) => {
    ...
    return (
        <div id={"window-" + props.id} className="window" onMouseDown={props.focus} ref={draggableRef} style={{
            top: props.appState === AppState.Maximized ? 0 : props.pos.top,
            left: props.appState === AppState.Maximized ? 0 : props.pos.left,
            ...
            width: props.appState === AppState.Maximized ? "100%" : props.pos.right - props.pos.left,
            height: props.appState === AppState.Maximized ? "100%" : props.pos.bottom - props.pos.top,
            ...
            <div className={["title-bar", !props.focused && "inactive"].join(" ")}>
                ...
                <div className="title-bar-controls" data-no-dnd="true">
                    {
                        (props.app.controls === Controls.All || !props.app.controls) && (
                            <>
                                <button aria-label="Minimize" onClick={() => props.setAppState(AppState.Minimized)}></button>
                                <button aria-label={props.appState === AppState.Maximized ? "Restore" : "Maximize"} onClick={() => props.setAppState(props.appState === AppState.Maximized ? AppState.Open : AppState.Maximized)}></button>
                            </>
                        )}
